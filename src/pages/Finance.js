...
const Finance = () => {
    ...
    const fetchFinanceUsers = async () => {
        ...
        setError(null);
        try {
            const result = await apiService.list(APIFinanceList);

            if (result.success && isMounted.current) {
                const usersData = Array.isArray(result.data) ? result.data : [];
                setFinanceUsers(usersData);
            } else {
                setError(result.message);
                showMessage(result.message || 'Failed to fetch finance users', 'error');
            }
        } catch (error) {
            console.error('Error fetching finance users:', error);
            setError("Failed to load data");
            showMessage('Error fetching finance users', 'error');
        } finally {
    ...
    const handleOpenDialog = (user = null) => {
        if (user) {
            ...
            const deptId = user.department_id || user.department || '';
    ...
    const handleSubmit = async () => {
        ...
        // Always send the department field with the department_id value
        ...
        try {
            let result;
            if (editingUser) {
                formDataToSend.append('updateId', editingUser.id);
                result = await apiService.update(APIFinanceUpdate, formDataToSend);
                if (result.success && isMounted.current) {
                    showMessage(`Finance user "${formData.name}" updated successfully`);
                }
            } else {
                result = await apiService.create(APIFinanceStore, formDataToSend);
                if (result.success && isMounted.current) {
                    showMessage(`Finance user "${formData.name}" created successfully`);
                }
            }
            ...
            if (result.success && isMounted.current) {
                ...
                if (result.errors) {
                    ...
                    showMessage(errorMessages || result.message || 'Failed to save finance user', 'error');
                } else {
                    showMessage(result.message || 'Failed to save finance user', 'error');
                }
            }
        } catch (error) {
            console.error('Error saving finance user:', error);
            showMessage('Error saving finance user', 'error');
        }
    };
    ...
    const handlePasswordChange = async () => {
        ...
        try {
            const formDataToSend = new FormData();
            formDataToSend.append('user_id', getUserDetails("Id"));
            formDataToSend.append('user_id', passwordUser.id);
            ...
            const result = await apiService.update(APIFinanceChangePassword, formDataToSend);
            
            if (result.success && isMounted.current) {
                showMessage(`Password updated successfully for "${passwordUser.name}"`);
    ...
    const confirmDelete = async () => {
        if (deleteConfirm) {
            ...
            try {
                const result = await apiService.delete(APIFinanceDelete, formDataToSend);
                if (result.success && isMounted.current) {
                    showMessage(`Finance user "${deleteConfirm.name}" deleted successfully`);
                    fetchFinanceUsers();
                    setDeleteConfirm(null);
                } else {
                    showMessage(result.message || 'Failed to delete finance user', 'error');
                }
            } catch (error) {
                console.error('Error deleting finance user:', error);
                showMessage('Error deleting finance user', 'error');
            }
        }
    };
    ...
    const filteredUsers = financeUsers.filter(user => {
        const matchesSearch = searchTerm === '' ||
            user.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            user.employee_number?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            user.email?.toLowerCase().includes(searchTerm.toLowerCase());
        
        const userDeptId = user.department_id || user.department;
        const matchesDepartment = !departmentFilter || String(userDeptId) === String(departmentFilter);
    ...
    const activeCount = financeUsers.filter(user => user.is_active).length;
    const inactiveCount = financeUsers.filter(user => !user.is_active).length;
    const departmentCount = new Set(financeUsers.map(user => user.department_id || user.department).filter(Boolean)).size;

    if (loading) {
        return (
            <LoadingState
                message="Loading Finance Users..."
                variant="welfare"
                size="large"
                fullPage
            />
    ...
    if (error) {
        return (
            <ErrorState
                error={error}
                onRetry={fetchFinanceUsers}
                title="Unable to Load Finance Users"
                variant="employee"
                fullPage
        ...
        <Box>
            <Paper sx={{ p: 3, borderRadius: 3, mb: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, flexWrap: 'wrap', gap: 2 }}>
                    <Typography variant="h5" sx={{ fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: 1 }}>
                        <AccountBalance sx={{ color: '#1565C0' }} />
                        Finance Management ({recordsCount(financeUsers.length)} Total)
                    </Typography>
                    <Button
                        variant="contained"
                        startIcon={<AddIcon />}
                ...
                <Box sx={{ mb: 3, display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                    <Chip 
                        icon={<People />}
                        label={`Total: ${financeUsers.length}`} 
                        color="primary" 
                        variant="outlined"
                    />
                    <Chip 
                            ...
                            {paginatedUsers.length === 0 ? (
                                ...
                                paginatedUsers.map((user, index) => (
                                    <TableRow key={user.id} hover>
                                        ...
                                        <TableCell>
                                            <Chip
                                                label={getDepartmentName(user.department_id || user.department)} 
                                                size="small"
                                                variant="outlined"
                                                color="primary"
                                            />
                                        ...
                                        <TableCell align="center">
                                            <Tooltip title="Edit Finance User">
                                            ...
                                            <Tooltip title="Delete Finance User">
            ...
            >
                <DialogTitle sx={{ textAlign: 'center' }}>
                    <b>{editingUser ? 'Edit Finance User' : 'Add New Finance User'}</b>
                </DialogTitle>
                <DialogContent>
                    <Box component="form" onSubmit={handleSubmit} sx={{ mt: 2 }}>
                                ...
                                <Box>
                                    ...
                                    <Grid item xs={12} sm={6}>
                                        <TextField
                                            ...
                                            onChange={handleInputChange}
                                            placeholder="e.g., Accountant, Finance Officer, Treasurer"
                                            InputProps={{
                                                startAdornment: (
                                                    <InputAdornment position="start">
                                                        <Work />
                                    ...
                                    <Grid item xs={12}>
                                        <Paper variant="outlined" sx={{ p: 2, bgcolor: '#f5f5f5' }}>
                                            <FormControlLabel
                                                ...
                                                label="Active Finance User"
                                            />
                                            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>
                                                Inactive users cannot access the finance system
                                            </Typography>
                                        </Paper>
                                    </Grid>
                                </Grid>
                        ...
                        <DialogActions sx={{ mt: 3, px: 0 }}>
                            ...
                            <Button onClick={handleSubmit} variant="contained" color="primary">
                                {editingUser ? 'Update Finance User' : 'Create Finance User'}
                            </Button>
                        </DialogActions>
                    </Box>
                </DialogContent>
                ...
                <DialogTitle sx={{ textAlign: 'center' }}>
                    ...
                    {passwordUser && (
                        <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                            Finance User: {passwordUser.name} ({passwordUser.employee_number})
                        </Typography>
                    )}
                </DialogTitle>
                <DialogContent>
            ...
            <Dialog
                ...
                disableEscapeKeyDown
            >
                <DialogTitle sx={{ textAlign: 'center' }}>Confirm Delete Finance User</DialogTitle>
                <DialogContent>
                    <Typography>
                        Are you sure you want to delete finance user <strong>"{deleteConfirm?.name}"</strong>?
                    </Typography>
                    <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                        Staff #: {deleteConfirm?.employee_number}
                    </Typography>
                    ...
                    <Alert severity="warning" sx={{ mt: 2 }}>
                        ...
                        This will permanently delete this finance user and all associated data.
                    </Alert>
                </DialogContent>
                <DialogActions>
